...
def main(args):
    ...
    print("Train:")
    train_pred = LR.predict(X_train_peaks_feat_extend_df)
    ...
    print("test:")
    test_pred = LR.predict(X_test_peaks_feat_extend_df)
    ...
    cur_f_beta = fbeta_score(y_test, LR.predict(X_test_peaks_feat_extend_df), beta=2) 
    print(f'F_beta score: {cur_f_beta}') 
    ...
    print("Train score", round(LR.score(X_train_peaks_feat_extend_df, y_train), 4))
    print("Test score", round(LR.score(X_test_peaks_feat_extend_df, y_test), 4))

    y_test_pred = LR.predict(X_test_peaks_feat_extend_df)
    print()
    C_LR = confusion_matrix(y_test, y_test_pred)
    show_validation_results(C_LR, total_time)    
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--window', type=int, default=38)
